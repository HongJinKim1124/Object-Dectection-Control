...
class Tello:
    ...
    def __init__(self, local_ip, local_port, imperial=False, command_timeout=.3, tello_ip='192.168.10.1',
        ...
        self.response = None  
        self.frame = None  # Numpy 배열 RGB - 현재 카메라 출력 프레임
        self.is_freeze = False  # 현재의 카메라 출력을 동결하다.
        self.last_frame = None
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # cmd 전송용 소켓
        self.socket_video = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # 비디오 스트림 수신용 소켓
        self.tello_address = (tello_ip, tello_port)
        self.local_video_port = 11111  # 비디오 스트림 수신 포트
        self.last_height = 0
    ...
    def _h264_decode(self, packet_data):
        ...
        for framedata in frames:
            ...
            if frame is not None:
                # '프레임 크기 %i바이트, w %i, h %i, 라인 크기 %i' %(렌즈), w, h, ls) 인쇄
